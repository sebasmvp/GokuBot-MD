const handler = async (m, {conn}) => {
  try {
    const pp = imagen6;
    const img = await(await fetch('https://github.com/Jostin207.png')).buffer();
    const _uptime = process.uptime() * 1000;
    const uptime = clockString(_uptime);
    const taguser = '@' + m.sender.split('@s.whatsapp.net')[0];
    const str = `╭────[ Ⓖ ✨💖𝙶𝙾𝙺𝚄𝙱𝙾𝚃-𝙼𝙳💖✨]
│
│ *➤ ʜᴏʟᴀ ${taguser}*
│
│ *=> 🖥️ ᴛɪᴇᴍᴘᴏ ᴀᴄᴛɪᴠᴏ:* ${uptime} ✓
│ *=> 🖱️ ʙᴏᴛ ᴅᴇ ᴜsᴏ ᴘᴜʙʟɪᴄᴏ* ✓
│ *=> 🍇 𝐂𝐑𝐄𝐀𝐃𝐎𝐑: ᴊᴏsᴛɪɴ ᴏғɪᴄɪᴀʟ* ✓
│ *=> 🍁 ʀᴇᴘᴏsɪᴛᴏʀɪᴏ ᴏғᴄ:* https://│github.com/Jostin207/GokuBot-MD
╰────────────────`.trim();
    if (m.isGroup) {
